    ...
    <main role="main" class="mt-3">
      <div class="container-fluid">
          ...
          // $prod_id=$output['product_id'];
              ...
              $sql = "SELECT o.customer_id customer,p.id product,r.fraction, r.selling_price as selling_price, r.id, p.name as product_name,p.supplier_id as supplier,p.unit_price as cost,p.description as description, p.images as images, p.color as color,o.units as units
              FROM ready_sale r 
              INNER JOIN products p ON r.product_id = p.id join orders o on(p.id=o.product_id) where o.customer_id=$user_id and o.status='unconfirmed' group by o.customer_id,product,status";
                  ...
                  <figure class='media'>
                    ...
                    <td> 
                      <div class='price-wrap'> 
                        <form method='POST'>
                           <input type='number' name='qnty' value='{$row['units']}'>
                           <input type='hidden' name='prod' value='{$row['product']}'>

                           <input type='submit' name='updateqnty' class='btn btn-outline-success delete-btn' value='update'>
                        </form>
                       </div>
                    </td>
                    <td> 
                    ...
                    <form method='POST'>
                           <input type='hidden' name='prod' value='{$row['product']}'>
                        <a><input type='submit' name='remove' class='btn btn-outline-danger delete-btn' value='× Remove'></a>
                    </form>
                    </td>
                  </tr>";
                  }
    ...
    <script>
      ...
      <?php
           if(isset($_POST['remove'])){
            $prodid=$_POST['prod'];
            echo $user_id;
            $qrydel="delete from orders where product_id='$prodid' and customer_id=$user_id";
           ...
           if(isset($_POST['updateqnty'])){
            $qnty=$_POST['qnty'];
            $prodid=$_POST['prod'];
            $qrydel="update orders set units='$qnty' where product_id='$prodid' and customer_id=$user_id";
            $resdel=mysqli_query($conn,$qrydel);
